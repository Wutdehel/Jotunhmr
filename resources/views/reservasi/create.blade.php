...
@error('tgl_reservasi_wisata') is-invalid @enderror" id="tgl_reservasi_wisata" placeholder="Tanggal Pemesanan" name="tgl_reservasi_wisata" value="{{old('tgl_reservasi_wisata')}}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control @error('harga') is-invalid @enderror" id="harga" placeholder="Harga"  name="harga" value="{{old('harga')}}" onchange = "hargadisk()" readonly>
                        @error('harga') <span class="textdanger">{{$message}}</span> @enderror
                    </div>

                    <div class="form-group">
                        <label for="diskon">Diskon Dalam Persenan</label>
                        <input type="number" class="form-control @error('diskon') is-invalid @enderror" id="diskon" placeholder="Diskon" max='100' name="diskon" value="{{old('diskon')}}" onchange = "hargadisk()" readonly>
                        @error('diskon') <span class="textdanger">{{$message}}</span> @enderror
                    </div>

                    <div class="form-group">
                        <label for="nilai_diskon">Nilai Diskon</label>
                        <input type="number" class="form-control @error('nilai_diskon') is-invalid @enderror" id="nilai_diskon" placeholder="Nilai Diskon"  name="nilai_diskon" value="{{old('nilai_diskon')}}" readonly>
                        ...
                        <table class="table table-hover table-bordered tablestripped" id="example2">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-primary
btn-xs" onclick="pilih1('{{$a->id}}', '{{$a->nama_paket}}', '{{$a->diskon}}', '{{$a->harga_per_pack}}'); hargadisk(); " data-bs-dismiss="modal">
                                            Pilih
                                        </button>
                                    </td>
                                </tr>
        ...
        <script>
            ...
            function pilih1(id_paket, paket_wisata, harga_per_pack, diskon) {
                document.getElementById('id_paket').value = id_paket
                document.getElementById('paket_wisata').value = paket_wisata 
                document.getElementById('harga').value = diskon
                document.getElementById('diskon').value = harga_per_pack
